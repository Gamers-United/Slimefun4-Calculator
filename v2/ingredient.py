...
class Ingredient:
    ...
    def addRecipe(self, ingredients: [], qty = 1.0, process_host: str = "minecraft:crafting_table"):
        from recipe import Recipe
        self.recipe = Recipe(ingredients, self, float(qty), process_host)
        return self

    def addShapedRecipe(self, shaped_recipe: [], qty = 1.0, process_host: str = "minecraft:crafting_table"):
        from recipe import Recipe
        ingredients = []
        ingredientHolder = {}
        ...
        self.recipe = Recipe(ingredients, self, float(qty), process_host, is_shaped=True, shaped_recipe=shaped_recipe)
        return self
    ...
    def addAquisitionDescriptor(self, gather_type, description):
        if not self.is_base:
            raise exceptions.AquisitonWrongType()
        else:
